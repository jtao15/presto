...
public class TestSqlTask
{
    ...
    public void testEmptyQuery()
    {
        ...
        taskInfo = sqlTask.getTaskInfo();
        assertEquals(taskInfo.getTaskStatus().getState(), TaskState.FINISHED);
        checkEvents(tracerEvents, new TracerEventType[] {TASK_STATE_CHANGE_RUNNING, TASK_STATE_CHANGE_FINISHED});
    }
    ...
    public void testCancel()
    {
        ...
        assertEquals(taskInfo.getTaskStatus().getState(), TaskState.CANCELED);
        assertNotNull(taskInfo.getStats().getEndTime());
        checkEvents(tracerEvents, new TracerEventType[] {TASK_STATE_CHANGE_RUNNING, TASK_STATE_CHANGE_CANCELED});
    }
            ...
            throws Exception
    {
        ...
        checkEvents(tracerEvents, new TracerEventType[] {TASK_STATE_CHANGE_RUNNING, TASK_STATE_CHANGE_FAILED});
    }
    ...
    private void checkEvents(List<TracerEvent> tracerEvents, TracerEventType[] expectedEventTypes)
    {
        try {
        ...
        Map<String, Boolean> eventOccurrence = new HashMap<>();
        for (TracerEventType eventType : expectedEventTypes) {
            eventOccurrence.put(eventType.name(), false);
        }

        for (TracerEvent event : tracerEvents) {
            if (eventOccurrence.containsKey(event.getEventType())) {
                eventOccurrence.put(event.getEventType(), true);
            }
        }

        for (Map.Entry<String, Boolean> eventEntry : eventOccurrence.entrySet()) {
            assertTrue(eventEntry.getValue(), "task should have action: " + eventEntry.getKey());
        }
    }
            ...
            implements TracerFactory
    {
        ...
        {
            return DefaultTracer.createBasicTracer(event -> this.tracerEvents.add(event), queryId, true);
        }
    }
}
