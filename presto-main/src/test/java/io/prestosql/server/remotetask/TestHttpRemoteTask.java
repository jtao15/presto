...
public class TestHttpRemoteTask
{
    ...
    private RemoteTask createRemoteTask(HttpRemoteTaskFactory httpRemoteTaskFactory, List<TracerEvent> tracerEvents)
    {
        ...
        Tracer taskTracer = DefaultTracer.createBasicTracer(event -> tracerEvents.add(event), payloadMap -> jsonCodec.toJson(payloadMap), "node", URI.create("http://test.com"), taskId.getQueryId().getId(), true)
