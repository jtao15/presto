        ...
        implements TracerFactory
{
    ...
    {
        return DefaultTracer.createBasicTracer(event -> eventListenerManager.tracerEventOccurred(event), payloadMap -> jsonCodec.toJson(payloadMap), nodeInfo.getNodeId(), uri, queryId, isQueryDebuggingTracerEnabled(session));
    }
}
