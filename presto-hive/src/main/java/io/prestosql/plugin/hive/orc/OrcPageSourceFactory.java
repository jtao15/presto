...
import static io.prestosql.plugin.hive.tracer.HiveTracerEventType.OPEN_FILE_SYSTEM_INPUT_STREAM_END;
import static io.prestosql.plugin.hive.tracer.HiveTracerEventType.OPEN_FILE_SYSTEM_INPUT_STREAM_START;
        ...
        implements HivePageSourceFactory
{
            ...
            ConnectorOperationContext connectorOperationContext)
    {
        ...
        try {
            ...
            connectorOperationContext.getConnectorTracer().ifPresent(tracer -> tracer.emitEvent(OPEN_FILE_SYSTEM_INPUT_STREAM_START, null));
            FSDataInputStream inputStream = hdfsEnvironment.doAs(sessionUser, () -> fileSystem.open(path));
            connectorOperationContext.getConnectorTracer().ifPresent(tracer -> tracer.emitEvent(OPEN_FILE_SYSTEM_INPUT_STREAM_END, null));
        ...
        try {
            OrcReader reader = new OrcReader(orcDataSource, options, connectorOperationContext);
